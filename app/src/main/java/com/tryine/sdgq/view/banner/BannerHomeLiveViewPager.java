...
public class BannerHomeLiveViewPager extends RelativeLayout implements ViewPager.OnPageChangeListener {
    private View mLayout;//布局
    private Activity mContext;//上下文
    private ViewPager mViewPager;//viewpager
    private BannerHomeLivePagerAdapter mPagerAdapter;//adapter
    private LinearLayout mLineIndicator;//指示器集合容器
    private ImageView[] mImageView;//小圆点imageview对象
    private List<LiveBean> mList;//url数组
    ...
    private BannerHomeLiveViewPager.OnClickBannerListener mBannerListener;

    public BannerHomeLiveViewPager addBannerListener(BannerHomeLiveViewPager.OnClickBannerListener listener) {
    ...
    private BannerHomeLiveViewPager.OnPageSelectedListener mBannerPageSelectedListener;

    public BannerHomeLiveViewPager addOnPageSelected(BannerHomeLiveViewPager.OnPageSelectedListener listener) {
...
//        }
//    };
    public BannerHomeLiveViewPager(Context context) {
        super(context);
    }

    public BannerHomeLiveViewPager(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public BannerHomeLiveViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     */
    public BannerHomeLiveViewPager initBanner(List<LiveBean> list, boolean isGallery) {
        mList = list;
        //引入布局
        ...
        mPagerAdapter = new BannerHomeLivePagerAdapter(mList, mContext);
        mPagerAdapter.setOnClickImagesListener(new BannerHomeLivePagerAdapter.OnClickImagesListener() {
            @Override
     ...
     * @param alpha     滑动透明度变化
     */
    public BannerHomeLiveViewPager initBanner(List<LiveBean> list, boolean isGallery, float alpha) {
        mList = list;
    ...
    public void refreshBanner(List<LiveBean> list){
        mList = list;
        mPagerAdapter.refreshBanner(mList);
    }
     ...
     * @return
     */
    public BannerHomeLiveViewPager addDefaultImg(int resId_img) {
     ...
     * @return
     */
    public BannerHomeLiveViewPager addRoundCorners(int corners) {
     ...
     */
    public BannerHomeLiveViewPager addPageMargin(int columnMargin, int rowMargin) {
     ...
     * @param distance 间距
     */
    public BannerHomeLiveViewPager addPoint(int distance) {
        isPoint = true;
     ...
     * @param piont       替换未选中图片
     */
    public BannerHomeLiveViewPager addPoint(int distance, int piont_press, int piont) {
        isPoint = true;
     ...
     * @param paddBottom
     */
    public BannerHomeLiveViewPager addPointBottom(int paddBottom) {
     ...
     * 配置完成,将布局添加到父容器
     */
    public BannerHomeLiveViewPager finishConfig() {
        this.addView(mLayout);
        return this;
    }

    //开始轮播
    public BannerHomeLiveViewPager addStartTimer(int time) {
        mRollTime = time;
        if (mHandler == null) {
